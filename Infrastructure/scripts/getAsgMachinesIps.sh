...
do
    ...
    sudo ssh -oStrictHostKeyChecking=no -i "/home/runner/candidate.pem" ubuntu@$IP 'touch $HOME/accessKey.txt; echo $ACC_KEY > $HOME/accessKey.txt' > /dev/null 2>&1
    sudo ssh -oStrictHostKeyChecking=no -i "/home/runner/candidate.pem" ubuntu@$IP 'touch $HOME/secAccessKey.txt; echo $SEC_ACC_KEY > $HOME/secAccessKey.txt' > /dev/null 2>&1
